...
- `tool_call_args`: one row per allowlisted tool call parameter (per chunk), for `--arg tool:key=value` filtering
...
- for tools in `TOOL_ARG_ALLOWLIST` (`packages/sesame/tool-arg-allowlist.ts`), allowlisted parameters are also stored in `tool_call_args` (migration `007`), one row per (chunk, key), powering the `--arg tool:key=value` filter; the allowlist covers pi's native tools plus the caller's custom tools, and parameter text stays searchable via FTS regardless
...
erDiagram
  ...
  chunks ||--o{ tool_call_args : parameterized_by
...
- `idx_tool_call_args_chunk`
- `idx_tool_call_args_key`
